...
public class Dp62 {
	...
	public static void optimalStops(int[] input) {
		...
		for (int i = 0; i < hotel.length; i++) {
			System.out.println("checking for " +i+"--"+hotel[i]);
			int h=(int)Math.pow((200 - hotel[i]), 2);
			bestpath[i] = (int)h ;
			stop[i] = 0;
			for (int j = 0; j < i; j++) {
				System.out.println("comparing distance of hotel "+j+"--"+hotel[j]);
				// is penalty of j + this new penalty less than current one which i has
				if (bestpath[j] + Math.pow((200 - (hotel[i] - hotel[j])), 2) < bestpath[i]) {
					bestpath[i] = (int) (bestpath[j] + Math.pow(	(200 - (hotel[i] - hotel[j])), 2));
					stop[i] = j + 1; // then update stop location of i to j [or j+1]
					System.out.println("updated bestpath->"+Arrays.toString(bestpath));
				}else {
	...
	public static void main(String[] args) {
		...
		int[] input = {190, 200,420, 550,556, 600,800 };
		Dp62.optimalStops(input);
