...
public class ParallelCourses {
	...
	public static void main(String args[]) {
		...
		int n = 3; int [][] relations =new int[][] {{1,3},{2,3}};
		p.minimumSemesters(n,relations);
	    ...
	    public int minimumSemesters(int N, int[][] relations) {
	        ...
	        for (int node = 1; node < N + 1; node++) {
	            // if has cycle, return -1
	          	dfsCheckCycle2(node, graph, visited,1);
	        ...
	        System.out.println(maxLength);System.out.println(maxtext);
	        return maxLength;
	    }
	    ...
	    int maxtext=0;
	    private int dfsCheckCycle2(int node, List<List<Integer>> graph, int[] visited, int test) {
	        // return -1 if has a cycle
	        // return 1 if does not have any cycle
	    	
	        if (visited[node] != 0) {
	        	maxtext=Math.max(test, maxtext);
	            return visited[node];
	        } else {
	            // mark as visiting
	        	maxtext=Math.max(test, maxtext);
	        ...
	        for (int endNode : graph.get(node)) {
	            if (dfsCheckCycle2(endNode, graph, visited,test+1) == -1) {
	                // we meet a cycle!
	                return -1;
	            }
	        }
